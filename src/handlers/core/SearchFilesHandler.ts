...
export class SearchFilesHandler extends BaseToolHandler {
  async runTool(args: unknown, drive: drive_v3.Drive): Promise<CallToolResult> {
    const validArgs = SearchFilesSchema.parse(args);

    console.error(
      `[${new Date().toISOString()}] [DEBUG] MCP-Google-Drive: Searching for: "${validArgs.query}"`
    );
  ...
  ): Promise<Array<drive_v3.Schema$File & { relevanceScore: number }>> {
    try {
      // Generate search variations for better matching
      const searchVariations = this.generateSearchVariations(args.query);
      console.error(
        `[${new Date().toISOString()}] [DEBUG] MCP-Google-Drive: Generated ${searchVariations.length} search variations`
      );
      ...
      for (const variation of searchVariations) {
        try {
          ...
          console.error(
            `[${new Date().toISOString()}] [DEBUG] MCP-Google-Drive: Searching with query: "${query}"`
          );
          ...
          console.error(
            `[${new Date().toISOString()}] [WARN] MCP-Google-Drive: Search variation failed: "${variation}"`,
            error
          );
      ...
      console.error(
        `[${new Date().toISOString()}] [INFO] MCP-Google-Drive: Found ${results.length} files (from ${allFiles.length} total)`
      );
  ...
  private formatSearchResults(
    files: Array<drive_v3.Schema$File & { relevanceScore: number }>,
