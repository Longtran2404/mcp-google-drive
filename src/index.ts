  ...
  try {
    ...
    if (args.includeContent && response.data.mimeType?.includes('text')) {
      try {
        ...
        console.error('Failed to get file content:', contentError);
      }
    }
...
async function createFile(args: z.infer<typeof CreateFileSchema>) {
  try {
    ...
    const media = {
      mimeType: args.mimeType,
      body: Buffer.from(args.content, 'utf-8'),
    };
...
console.error('MCP Google Drive server started with enhanced features');
